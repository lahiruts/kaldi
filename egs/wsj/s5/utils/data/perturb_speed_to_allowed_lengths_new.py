...
def read_kaldi_datadir(dir):
    ...
    text = read_kaldi_mapfile(os.path.join(dir, 'text'), encode='utf-8')
    utt2dur = read_kaldi_mapfile(os.path.join(dir, 'utt2dur'))
    utt2spk = read_kaldi_mapfile(os.path.join(dir, 'utt2spk'))


    logger.info("wav_scp length {}".format(len(wav_scp)))
    logger.info("text length {}".format(len(text)))
    logger.info("utt2dur length {}".format(len(utt2dur)))
    logger.info("utt2spk length {}".format(len(utt2spk)))
    ...
    for utt in wav_scp:
        if utt in text and utt in utt2dur and utt in utt2spk:
            ...
            num_fail += 1
    logger.info("number of utterances {}".format(len(utterances)))
    logger.info("number of failures {}".format(num_fail))
...
def read_kaldi_mapfile(path, encode='latin-1'):
...
def generate_kaldi_data_files(utterances, outdir):
    ...
    with open(os.path.join(outdir, 'text'), 'w', encoding='utf-8') as f:
...
def find_duration_range(utterances, coverage_factor):
    ...
    #if start_dur < 0.3:
    if start_dur < 1.0:
        start_dur = 1.0  # a hard limit to avoid too many allowed lengths --not critical
