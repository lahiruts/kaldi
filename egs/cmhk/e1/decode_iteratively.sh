...
if [ $stage -le 6 ]; then
  for val in 500 1000 2000 3000 4000 5000 6000;do
  echo $val
  #for test_set in test_01_hires test_02_hires test_03_hires test_04_hires val_hires; do
  for test_set in test_01_hires; do
    steps/nnet3/decode.sh --max-active $val --acwt 1.0 --post-decode-acwt 10.0 \
      --nj 20 --cmd "$decode_cmd" \
      $graph data/${test_set} $dir/decode_${test_set}_max_active_${val} || exit 1;
  done
  done
fi
