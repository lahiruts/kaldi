...
devset=val
#testset=test_01 test_02 test_03 test_04
pythoncmd=python3.7
numChars=3000
...
if [ $stage -le 0 ]; then
    for x in train val test_01 test_02 test_03 test_04; do
...
if [ $stage -le 4 ]; then
  ...
  if [ -f data/test_hires/feats.scp ]; then
    ...
    for x in test_01 test_02 test_03 test_04 val; do
...
if [ $stage -le 5 ]; then
#extracting wav segments
utils/data/extract_wav_segments_data_dir.sh data/${trainset} data/${trainset}_spe2e
fi
...
if [ -f data/${trainset}_spe2e_hires/feats.scp ]; then
  ...
  if [ $stage -le 6 ]; then
    ...
    $pythoncmd utils/data/perturb_speed_to_allowed_lengths_new.py 12 data/${trainset}_spe2e \
