...
test_sets="test_01_hires val_hires test_02_hires test_03_hires test_04_hires"
...
dir=exp/chain/e2e_tdnnf_char${affix}
...
if [ $stage -le 0 ]; then
  ...
  # topology.
  e2e/gen_topo_one.py $nonsilphonelist $silphonelist >$lang/topo
fi
...
if [ $stage -le 3 ]; then
  ...
  python2.7 steps/nnet3/chain/e2e/train_e2e.py --stage $train_stage \
    --cmd "$decode_cmd" \
    ...
    (
      ...
      for lmtype in 4g; do
        steps/nnet3/decode.sh \
          ...
          $treedir/graph_${lmtype} data/${data} ${dir}/decode_${lmtype}_${data} || exit 1
      done
    ) || touch $dir/.error &
  done
  wait
